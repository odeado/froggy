...
export class Lane {
  ...
  // Se da un pequeño margen de perdón (la hitbox es algo más chica que el
  // sprite dibujado) para que el choque se sienta justo, no "invisible".
  collidesWithCell(col, row) {
    ...
    for (const v of this.vehicles) {
      ...
      for (const offset of offsets) {
        const start = v.xCells + offset + forgiveness;
        const end = start + v.lengthCells - forgiveness * 2;
